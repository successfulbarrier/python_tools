...
from yolo_model_load.common import *
from yolo_model_load.experimental import *
from yolo_model_load.autoanchor import check_anchor_order
from yolo_dataset_load.general import LOGGER, check_version, check_yaml, make_divisible, print_args
from yolo_model_load.plots import feature_visualization
from yolo_model_load.torch_utils import fuse_conv_and_bn, initialize_weights, model_info, scale_img, select_device, time_sync
...
class DoubleConv(nn.Module):
    def __init__(self, c1, c2, nc):
        super().__init__()
        self.conv2d = nn.Conv2d(c1, c2, 1, bias=False)
        self.linear = nn.Linear(c2, c1)
        self.cls = nn.Linear(c1, nc * 3)
        self.loc = nn.Linear(c1, 15)
    ...
    def forward(self, x):
        ...
        classes = self.cls(x)
        location = self.loc(x)
        x = torch.cat([location, classes], dim=3)
        x = x.transpose(1, 3)
        return x
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        m = self.model[-1]  # Detect() model的最后一层是检测层
    ...
    def info(self, verbose=False, img_size=1024):  # print model information
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, MRT, MRE, DC, MRA, HRFF, SEModule, Up, TConv, Conv2,
                 E_context, Noise_filter, Noise_filter_2, ACmix]:
            c1, c2 = ch[f], args[0]
        ...
        elif m in [ACDetect, Detect, STDetect]:
