...
class TaskAlignedAssigner(nn.Module):
    ...
    def forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt):
        """
        Compute the task-aligned assignment.,标签分配策略,此函数并未对预测值做任何处理只是生成了标签信息
        ...
        """
        self.bs = pd_scores.size(0)   # 这一个batch的图片数量
        self.n_max_boxes = gt_bboxes.size(1)    # 单张照片上最多的目标数
    ...
    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, mask_gt):
        ...
        ind[1] = gt_labels.squeeze(-1)  # b, max_num_obj torch.squeeze() 删除张量中所有维度为一的维度
        ...
        return align_metric, overlaps   # 匹配程度，CIOU
...
class v8DetectionLoss:
    ...
    def __init__(self, model):  # model must be de-paralleled
        ...
        self.assigner = TaskAlignedAssigner(topk=10, num_classes=self.nc, alpha=0.5, beta=6.0)  # num_classes=self.nc  修改
        self.bbox_loss = BboxLoss(m.reg_max - 1, use_dfl=self.use_dfl).to(device)   # m.reg_max - 1   修改
    ...
    def __call__(self, preds, batch):
        ...
        函数功能：计算目标检测的损失，计算 box、cls 和 dfl 的损失之和乘以批量大小。
        preds 预测结果
            preds 的内容：一个列表有三个tensor矩阵
            0:(batch, channel, 80, 80) channel=144
            1:(batch, channel, 40, 40)
            2:(batch, channel, 20, 20)
            最终执行了 self._predict_once() 函数
        batch 标签信息
            batch 是一个字典包含七种属性
            'im_file' 图片路径
            'ori_shape' 图片原始尺寸
            'resized_shape' 变化后的尺寸
            'img' 图像内容信息
            'cls' 整个batch所有目标的类别信息
            'bboxes' 整个batch所有目标的坐标信息
            'batch_idx' 类别信息和坐标信息对应的图片索引列表
        """
        """
            我的测试程序
        """
        ...
        loss = torch.zeros(3, device=self.device)  # box, cls, dfl，定义损失
        feats = preds[1] if isinstance(preds, tuple) else preds  # feats = preds
        pred_distri, pred_scores = torch.cat([xi.view(feats[0].shape[0], self.no, -1) for xi in feats], 2).split(
            (self.reg_max * 4, self.nc), 1)     # 分割预测的数据 preds -> pred_distri (4, 64, 8400) pred_scores (4, 80, 8400)

        pred_scores = pred_scores.permute(0, 2, 1).contiguous()     # 交换通道 pred_scores (4, 8400, 80)
        pred_distri = pred_distri.permute(0, 2, 1).contiguous()     # 交换通道 pred_scores (4, 8400, 64)

        dtype = pred_scores.dtype   # 预测数据的类型
        batch_size = pred_scores.shape[0]   # 获取 batch_size 大小
        ...
        # target获取标签信息 (65,6) -> (目标个数, [对应的图片，目标类别，中心点x，中心点y，左上角x，左上角y])
        # targets = torch.cat((batch['batch_idx'].view(-1, 1), batch['cls'].view(-1, 1), batch['bboxes']), 1)
        targets = batch    # 修改

        # target : (4,26,5) -> (对应那一张图片，26个目标不够用0补齐，[cls, x, y, x, y])
        targets = self.preprocess(targets.to(self.device), batch_size, scale_tensor=imgsz[[1, 0, 1, 0]])
        gt_labels, gt_bboxes = targets.split((1, 4), 2)  # cls, xyxy
        mask_gt = gt_bboxes.sum(2, keepdim=True).gt_(0)  # 生成掩码，有效位置为1，无效位置为0

        # pboxes，求出相对于每个grid cell 中点的偏移坐标 (4, 8400, 64) -> (4, 8400, 4)
        pred_bboxes = self.bbox_decode(anchor_points, pred_distri)  # xyxy, (b, h*w, 4)

        """
        标签分配策略 --> ultralytics/yolo/utils/tal.py/TaskAlignedAssigner.forward()
        pred_scores.detach().sigmoid() 所有类别的置信度
        (pred_bboxes.detach() * stride_tensor).type(gt_bboxes.dtype) 相对偏移
        anchor_points * stride_tensor 恢复到原图每个预测框对应的真是图片中的中心点，box的中心点坐标会相对于这个进行偏移
        gt_labels 标签类别信息
        gt_bboxes 标签框  
        mask_gt 标签掩码
        """
