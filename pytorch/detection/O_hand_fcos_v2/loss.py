...
from tools import coords_fmap2orig
...
class GenTargets(nn.Module):
    ...
    def forward(self,inputs):
        ...
        '''
        # 得到输入
        cls_logits,cnt_logits,reg_preds=inputs[0]   # 网络输出
        gt_boxes=inputs[1]    # 标签坐标
        classes=inputs[2]   # 标签类别
        cls_targets_all_level=[]
        cnt_targets_all_level=[]
        reg_targets_all_level=[]
        assert len(self.strides)==len(cls_logits)
        for level in range(len(cls_logits)):
            # level_out是我们每一层的3个tensor的输出
            level_out=[cls_logits[level],cnt_logits[level],reg_preds[level]]
            # 根据我们的GT还有预测出来的tensor运算，处理一个特征层输出的 cls_logits cnt_logits reg_preds
    ...
    def _gen_level_targets(self,out,gt_boxes,classes,stride,limit_range,sample_radiu_ratio=1.5):
        ...
        gt_boxes [batch_size,m,4]  这里的box是左上角坐标和右下角坐标的格式
        ...
        m=gt_boxes.shape[1]   # 每张图片上目标的数量

        cls_logits=cls_logits.permute(0,2,3,1)   # [batch_size,h,w,class_num]，交换通道  
        ...
        cnt_logits=cnt_logits.permute(0,2,3,1)  # [batch_size,h,w,1]，交换通道
        cnt_logits=cnt_logits.reshape((batch_size,-1,1))
        reg_preds=reg_preds.permute(0,2,3,1)    # [batch_size,h,w,4]，交换通道
        reg_preds=reg_preds.reshape((batch_size,-1,4))

        h_mul_w=cls_logits.shape[1]     # 输出特征层长乘宽
