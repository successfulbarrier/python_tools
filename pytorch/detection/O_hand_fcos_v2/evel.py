...
import sys
sys.path.append("H:\\code\\python_tools\\pytorch\\detection\\O_hand_fcos")
...
class COCOGenerator(CocoDetection):
    ...
    CLASSES_NAME = (
        '__back_ground__', 'aeroplane', 'bicycle', 'bird', 'boat', 'bottle', 'bus', 'car', 'cat',
         'chair', 'cow', 'diningtable', 'dog', 'horse', 'motorbike', 'person',
         'pottedplant', 'sheep', 'sofa', 'train', 'tvmonitor')
    
    def __init__(self, imgs_path, anno_path, resize_size=[640, 800]):
...
if __name__ == "__main__":
    generator = COCOGenerator("J:\\code\\datasets\\VOC_s\\val\\images", "J:\\code\\datasets\\VOC_s\\val\\annotations\\val.json")
    model = FCOSDetector(mode="inference")
    model = torch.nn.DataParallel(model)
    model = model.cuda().eval()
    model.load_state_dict(torch.load("J:\\实验数据\\VOC_s实验结果\\O_hand_fcos_v2\\resnet18_50epoch_C3_16-p\\model_50.pth", map_location=torch.device('cpu')))
