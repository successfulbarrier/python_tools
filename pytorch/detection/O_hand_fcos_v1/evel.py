...
if __name__ == "__main__":
    generator = COCOGenerator("H:\\code\\datasets\\VOC_s\\val\\images", "H:\\code\\datasets\\VOC_s\\val\\annotations\\val.json")
    model = FCOSDetector(mode="inference")
    model = torch.nn.DataParallel(model)
    model = model.cuda().eval()
    model.load_state_dict(torch.load("H:\\实验数据\\VOC_s实验结果\\O_hand_fcos\\有预训练权重\\model_42.pth", map_location=torch.device('cpu')))
